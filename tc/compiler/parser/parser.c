...
static Node* parse_prefix(Parser *p) {
    ...
    switch (p->current.kind) {
        ...
        case TOK_LPAREN: {
            ...
            /* Check for cast: (type) expr where type is a keyword or typedef name.
             * To distinguish from (expr), we check if the token after the type name
             * is ')'. If the type keyword is followed by something other than ')'
             * (like an operator), it's a parenthesized expression.
             */
            if (is_base_type(p->current.kind) || is_type_qualifier(p->current.kind)
                || p->current.kind == TOK_IDENT) {
                /* Save lexer state for potential backtracking */
                LexerState lex_state;
                lexer_save_state(p->lexer, &lex_state);
                ...
                if (is_base_type(p->current.kind) || is_type_qualifier(p->current.kind)) {
                    type_name = p->current.ident;
                    type_hash = p->current.id_hash;
                    advance(p);
                    type_consumed = 1;
                    /* Consume additional qualifiers */
                    while (is_type_qualifier(p->current.kind) || is_base_type(p->current.kind)) {
                        advance(p);
                        type_consumed = 1;
                    }
                ...
                /* Not a cast - restore lexer state and treat as parenthesized expression */
                lexer_restore_state(p->lexer, &lex_state);
                p->current = save;
            }
...
static int parse_type_spec(Parser *p) {
    /* Consume type qualifiers (but NOT long/short which can be base types) */
    while (p->current.kind == TOK_KW_SIGNED || p->current.kind == TOK_KW_UNSIGNED
               || p->current.kind == TOK_KW_CONST
               || p->current.kind == TOK_KW_STATIC || p->current.kind == TOK_KW_EXTERN
               || p->current.kind == TOK_KW_REGISTER) {
        advance(p);
    }
    /* Consume base type and return spec_kind:
     *  0=int  1=void  2=char  3=short  4=long  (matches sema_build_type) */
    if (p->current.kind == TOK_KW_INT)   { advance(p); return 0; }
    if (p->current.kind == TOK_KW_VOID)  { advance(p); return 1; }
    if (p->current.kind == TOK_KW_CHAR)  { advance(p); return 2; }
    if (p->current.kind == TOK_KW_SHORT) { advance(p); return 3; }
    if (p->current.kind == TOK_KW_LONG)  { advance(p); return 4; }
    if (p->current.kind == TOK_KW_STRUCT || p->current.kind == TOK_KW_UNION
    ...
    /* Handle typedef names (identifiers used as type names, e.g. size_t) */
    if (p->current.kind == TOK_IDENT) {
        advance(p);
        return 0; /* treat as int for now, sema will resolve */
    }
...
static Node* parse_initializer(Parser *p) {
    ...
    int ninits = 0;
    int cap = 8;
    inits = (Node **)malloc(sizeof(Node *) * (size_t)cap);

    if (p->current.kind != TOK_RBRACE) {
        Node *init = parse_expr(p, 0);
        if (init) {
            inits[ninits++] = init;
            while (p->current.kind == TOK_COMMA) {
                advance(p);
                if (ninits >= cap) {
                    cap *= 2;
                    inits = (Node **)realloc(inits, sizeof(Node *) * (size_t)cap);
                }
                init = parse_expr(p, 0);
                if (init) {
                    inits[ninits++] = init;
    ...
    /* Create a NODE_BLOCK to hold the initializer list */
    Node *n = node_create(NODE_BLOCK, line, col);
    n->u.block.stmts = inits;
    n->u.block.nstmts = ninits;
    return n;
}
...
static Node* parse_declaration(Parser *p) {
    ...
    if (p->current.kind == TOK_LBRACKET) {
        advance(p);
        Node *size_expr = parse_expr(p, 0);
        if (size_expr && size_expr->is_const_expr) {
            array_size = (int)size_expr->u.lit_int.val;
        }
        if (p->current.kind == TOK_RBRACKET) {
            advance(p);
        }
    ...
    if (p->current.kind == TOK_EQ) {
        advance(p);
        /* Check for array initializer: { expr, expr, ... } */
        if (p->current.kind == TOK_LBRACE) {
            Node *init_list = parse_initializer(p);
            init = init_list;
            /* If no explicit array size, infer from initializer count */
            if (array_size == 0 && init_list) {
                array_size = init_list->u.block.nstmts;
            }
        } else {
            init = parse_expr(p, 0);
        }
    ...
    n->u.var_decl.ptr_stars = ptr_stars;
    n->u.var_decl.init = init;
    n->u.var_decl.is_static = is_static;
    n->u.var_decl.array_size = array_size;
    (void)hash;
    return n;
}
