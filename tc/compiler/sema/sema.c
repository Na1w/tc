...
{
    /* Use parser-provided spec_kind if available, fall back to int (spec_kind 0) */
    int spec_kind = node->u.var_decl.type_id;
    if (spec_kind < 0)
        spec_kind = 0;  /* default: int */

    /* Convert spec_kind to a type table entry */
    int type_id = sema_add_type(sema, sema_build_type(spec_kind, 0));

    /* Apply pointer stars to build the actual type (e.g., char -> char*) */
    for (int i = 0; i < node->u.var_decl.ptr_stars; i++) {
        type_id = sema_ptr_type(sema, type_id);
        if (type_id < 0)
            return -1;
    }

    /* Handle array declarations: char arr[4] or int arr[4] = {...} */
    int array_size = node->u.var_decl.array_size;
    if (array_size > 0) {
        /* Build array type: array[element_type][count] */
        Type *elem_type = sema_get_type(sema, type_id);
        Type *array_type = type_alloc();
        array_type->kind = TY_ARRAY;
        array_type->elem = elem_type;
        array_type->array_count = array_size;
        array_type->size = elem_type->size * array_size;
        array_type->sign = elem_type->sign;
        type_id = sema_add_type(sema, array_type);
    }

    int64_t init_value = 0;
    if (node->u.var_decl.init) {
        /* For array initializers (NODE_BLOCK), don't analyze as expression */
        if (node->u.var_decl.init->kind != NODE_BLOCK) {
    ...
    /* Only check CURRENT scope for redefinition - NOT the full scope chain.
     * A global variable 's' should NOT conflict with a function parameter 's'
     * in a different function. The fallback linear search in sema_lookup()
     * would find symbols from other scopes (e.g., function params) and
     * produce false "redefinition" errors. */
    ...
    Symbol *existing = NULL;
    {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            if (s) {
                ...
                break; /* empty slot - not found in this scope */
            }
        }
    }
    if (existing) {
...
static int analyze_func_decl(SemaContext *sema, Node *node)
{
    ...
    if (node->u.func_decl.ret_type_id >= 0) {
        ret_type_id = sema_add_type(sema, sema_build_type(node->u.func_decl.ret_type_id, 0));
    }
    ...
    func_type->is_variadic = node->u.func_decl.is_variadic;
...
static int analyze_expression(SemaContext *sema, Node *node)
{
    ...
    case NODE_CALL: {
        ...
        if (callee_t->kind == TY_FUNC && !callee_t->is_variadic) {
            int expected = callee_t->param_count;
            int actual   = node->u.call.nargs;
            if (actual < expected) {
                sema_error(sema,
                           "%zu:%zu: error: too few arguments in function call "
                           "(expected %d, got %d)",
                           node->line, node->col, expected, actual);
            } else if (actual > expected) {
                sema_error(sema,
                           "%zu:%zu: error: too many arguments in function call "
                           "(expected %d, got %d)",
                           node->line, node->col, expected, actual);
            }
        }
...
static int analyze_statement(SemaContext *sema, Node *node)
{
    ...
    case NODE_RETURN: {
        // Determine the enclosing function's return type
        // Use the func_type stored in current_func->type_id (set by analyze_func_decl)
        Type *func_ret_type = NULL;
        if (sema->current_func) {
            int func_type_id = sema->current_func->type_id;
            Type *func_type = sema_get_type(sema, func_type_id);
            if (func_type && func_type->kind == TY_FUNC && func_type->ret_type)
                func_ret_type = func_type->ret_type;
        }

        if (node->u.ret.value) {
            // There is a return value expression
            if (func_ret_type && func_ret_type->kind == TY_VOID) {
                sema_error(sema,
                           "%zu:%zu: error: void function cannot return a value",
                           node->line, node->col);
            } else {
            ...
            if (func_ret_type && func_ret_type->kind != TY_VOID) {
                sema_error(sema,
                           "%zu:%zu: error: non-void function must return a value",
                           node->line, node->col);
            }
        }
        return 0;
...
int sema_analyze(SemaContext *sema, Node *ast)
{
    ...
    /* Ensure we start at the global scope for top-level analysis */
    sema->current_scope = &sema->global_scope;

    if (ast->kind == NODE_BLOCK) {
        /* Top-level translation unit: process statements directly in global scope.
         * Do NOT create a local scope - that would put global vars in the wrong scope. */
        for (int i = 0; i < ast->u.block.nstmts; i++) {
            if (ast->u.block.stmts[i])
                analyze_statement(sema, ast->u.block.stmts[i]);
        }
    } else {
