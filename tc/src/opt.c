...
/* ==================================================================
 * Helper: replace all uses of old_temp with new_temp
 * ================================================================== */
static void replace_temp(IRProgram *prog, int old_temp, int new_temp)
{
    for (int i = 0; i < prog->count; i++) {
...
void opt_dead_code_elim(IRProgram *prog)
{
    ...
    int *worklist = malloc((size_t)prog->count * sizeof(int));
    int wl_top = 0;
...
void opt_cse_basic_block(IRProgram *prog)
{
    ...
    for (int i = 0; i < prog->count; i++) {
        ...
        /* A new basic block starts at: LABEL, or after BR (unconditional) */
        if (instr->kind == IR_LABEL) {
            ...
            /* Next non-NOP instruction starts a new BB */
        ...
        for (int i = start; i < end; i++) {
            ...
            for (int j = start; j < i; j++) {
                ...
                if (match) {
                    ...
                    replace_temp(prog, instr->temp_id, prev->temp_id);
                    instr->kind = IR_NOP;
                    break;
                }
            }
...
void opt_copy_propagation(IRProgram *prog)
{
    ...
    while (changed) {
        ...
        for (int i = 0; i < prog->count; i++) {
            ...
            /* Propagate: replace all uses of t1 with t2 */
            replace_temp(prog, t1, t2);
            instr->kind = IR_NOP;
            changed = 1;
        }
    }
